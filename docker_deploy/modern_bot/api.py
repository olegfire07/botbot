...
async def start_api_server(bot, host: str = None, port: int = None):
    ...
    async def handle_static(request):
        ...
        # Find web_app directory (same logic as handle_root)
        candidates = [
            Path(__file__).resolve().parent.parent / 'web_app',
            Path(__file__).resolve().parent / 'web_app'
        ]
        web_app_dir = None
        for path in candidates:
            if path.exists():
    ...
    app.router.add_get(r'/{filename:service-worker\.js|ux-improvements\.js}', handle_static)
