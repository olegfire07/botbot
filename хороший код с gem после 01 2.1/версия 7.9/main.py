...
# Важно: set_page_config должен быть первым вызовом Streamlit, прежде чем любые другие элементы.
...
############################
# Дополнительная функция для сохранения ML-модели в виде .pkl
############################
def save_trained_model_to_file(model, filename="trained_model.pkl"):
    """
    Сохраняет обученную модель в буфер (через joblib.dump),
    затем даёт возможность скачать её (кнопка st.download_button).
    """
    import io
    import joblib
...
def load_help_texts():
    """
    Загрузка help-текстов из help_text.yaml.
    Возвращает словарь с подсказками (или пустой словарь, если файл не найден).
    """
    try:
...
# Загружаем help-тексты в переменную
...
# Достаём некоторые значения из app_state
...
def reset_params():
    """
    Сбрасывает все параметры к настройкам по умолчанию из config.yaml,
    также сбрасывает загруженные файлы, и обнуляет метод forecast_method.
    """
...
# Пытаемся загрузить параметры из URL-параметра ?params=...
...
with st.sidebar:
    ...
    with st.sidebar.expander("### Настройка темы", expanded=False):
        ...
        main_color = st.color_picker(
            ...
            help="Выберите цвет, который будет использоваться в качестве основного."
        )
    ...
    with st.sidebar.expander("### Распределение площади", expanded=False):
        ...
        if mode == "Ручной":
            ...
            st.write("Площади распределяются автоматически в соответствии с заданными долями (в состоянии shares).")
    ...
    # --- Расширенные параметры и прогнозирование ---
    with st.sidebar.expander("### Расширенные параметры и прогнозирование", expanded=False):
        ...
        elif forecast_method_sel in ["ML (SVR)"] and enable_ml_settings_val:
            features_options = ["Месяц", "Lag_1", "Lag_2", "Rolling_Mean_3", "Rolling_Mean_5"]
            selected_features = st.multiselect(
                "Выберите признаки",
                options=features_options,
        ...
        if forecast_method_sel in ["ML (случайный лес)", "ML (SVR)"] and enable_ml_settings_val:
            ...
            auto_feature_selection_val = st.checkbox(
                ...
                help=help_texts.get("auto_feature_selection", ""),
            )
    ...
    # Загрузка данных для ML (CSV/Excel), если enable_ml_settings_val=True и метод - ML
    ...
    # Кнопка "Сохранить параметры" (в памяти session_state.saved_params)
    ...
    # Кнопка "Загрузить параметры (JSON/YAML)"
    ...
    # Сравнение сохранённых параметров
    ...
    # Кнопка "Сохранить в файл" (JSON/YAML)
    ...
    # Кнопка "Сформировать быстрый отчёт"
    if st.sidebar.button("Сформировать быстрый отчёт"):
        ...
        st.sidebar.write(f"**Суммарные единовременные расходы:** {one_time_sum:,.2f} руб.")
        st.sidebar.write(f"**Суммарные ежемесячные расходы:** {monthly_sum:,.2f} руб.")

    # Кнопка "Сформировать углублённый отчёт"
...
# Формируем объект WarehouseParams на основе данных из app_state
forecast_method = app_state.get("forecast_method") or "Базовый"
...
if not is_valid:
    ...
    # Добавляем новую вкладку ("Вкладка 5") для сценарного анализа (пример)
    tabs_list = ["Вкладка 1", "Вкладка 2", "Вкладка 3", "Вкладка 4", "Вкладка 5"]
    chosen = st.radio(
        "Вкладки:",
        tabs_list,
    ...
    if chosen == "Вкладка 1":
        ...
        if params.forecast_method == "Базовый":
            ...
            # Кнопка "Сохранить ML-модель" (если модель обучена)
            ...
            # Показываем базовый функционал с прогнозом
            display_tab2_ml_forecast(
                tab_container,
        ...
        ###########################
        # "Вкладка 5" — Расширенный сценарный анализ
        ###########################
        st.header("Сценарный анализ (Вкладка 5)")
        st.write(
            "Оптимистичный, Реалистичный и Пессимистичный сценарии для ключевых параметров. "
            "Сравним доход, расход и прибыль в каждом из них."
        )

        import copy
        scenario_options = ["Оптимистичный", "Реалистичный", "Пессимистичный"]
        selected_scenario = st.radio("Выберите сценарий:", scenario_options, index=1)

        temp_params = copy.deepcopy(params)

        if selected_scenario == "Оптимистичный":
            # Тариф хранения растёт на 20%, ставка займов падает на 20%
            temp_params.storage_fee *= 1.2
            temp_params.loan_interest_rate *= 0.8
            # Дополнительно уменьшим вероятность невозврата:
            temp_params.default_probability *= 0.8
        elif selected_scenario == "Пессимистичный":
            # Тариф хранения падает на 20%, ставка займов растёт на 20%
            temp_params.storage_fee *= 0.8
            temp_params.loan_interest_rate *= 1.2
            # Увеличим вероятность невозврата:
            temp_params.default_probability *= 1.3
        # Реалистичный - без изменений

        scenario_fin = calculate_financials(temp_params, disable_extended=False)
        scenario_irr = calculate_irr(
            [-temp_params.one_time_setup_cost
             - temp_params.one_time_equipment_cost
             - temp_params.one_time_other_costs
             - temp_params.one_time_legal_cost
             - temp_params.one_time_logistics_cost]
            + [scenario_fin["profit"]] * temp_params.time_horizon
        )
        scenario_pm, scenario_pr = calculate_additional_metrics(
            scenario_fin["total_income"],
            scenario_fin["total_expenses"],
            scenario_fin["profit"]
        )
        scenario_roi = calculate_roi(scenario_fin["total_income"], scenario_fin["total_expenses"])
        scenario_bep = calculate_total_bep(scenario_fin, temp_params)

        st.subheader(f"Результаты для сценария: {selected_scenario}")
        col1, col2, col3 = st.columns(3)
        col1.metric("Доход (руб.)", f"{scenario_fin['total_income']:,.2f}")
        col2.metric("Расход (руб.)", f"{scenario_fin['total_expenses']:,.2f}")
        col3.metric("Прибыль (руб.)", f"{scenario_fin['profit']:,.2f}")

        col4, col5, col6 = st.columns(3)
        col4.metric("Маржа прибыли (%)", f"{scenario_pm:.2f}")
        col5.metric("Рентабельность (%)", f"{scenario_pr:.2f}")
        col6.metric("ROI (%)", f"{(scenario_roi or 0.0):.2f}")

        col7, col8 = st.columns(2)
        col7.metric("IRR (%)", f"{scenario_irr:.2f}")
        if np.isinf(scenario_bep):
            col8.metric("BEP (руб.)", "∞")
        else:
            col8.metric("BEP (руб.)", f"{scenario_bep:,.2f}")

        st.info(
            "В Оптимистичном сценарии мы повысили тариф хранения и понизили ставку займов (и вероятность невозврата). "
            "В Пессимистичном наоборот, а также повысили вероятность невозврата (default_probability). "
            "Так мы минимизируем парадокс, когда пессимистичный сценарий случайно даёт большую прибыль."
        )

        # Дополнительный бар-чарт для сравнения 3 сценариев сразу
        st.write("---")
        st.subheader("Сравнение трёх сценариев (Доход/Расход/Прибыль)")
        scenario_labels = ["Оптимистичный", "Реалистичный", "Пессимистичный"]
        storage_fee_save = params.storage_fee
        loan_rate_save = params.loan_interest_rate
        default_prob_save = params.default_probability

        results = []
        for scenario_name in scenario_labels:
            scen_params = copy.deepcopy(params)
            if scenario_name == "Оптимистичный":
                scen_params.storage_fee *= 1.2
                scen_params.loan_interest_rate *= 0.8
                scen_params.default_probability *= 0.8
            elif scenario_name == "Пессимистичный":
                scen_params.storage_fee *= 0.8
                scen_params.loan_interest_rate *= 1.2
                scen_params.default_probability *= 1.3
            # Реалистичный - без изменений

            scen_fin = calculate_financials(scen_params, disable_extended=False)
            results.append({
                "Сценарий": scenario_name,
                "Доход": scen_fin["total_income"],
                "Расход": scen_fin["total_expenses"],
                "Прибыль": scen_fin["profit"]
            })

        # Восстанавливаем
        params.storage_fee = storage_fee_save
        params.loan_interest_rate = loan_rate_save
        params.default_probability = default_prob_save

        # Собираем DataFrame
        df_compare = []
        for r in results:
            df_compare.append({"Сценарий": r["Сценарий"], "Показатель": "Доход", "Значение": r["Доход"]})
            df_compare.append({"Сценарий": r["Сценарий"], "Показатель": "Расход", "Значение": r["Расход"]})
            df_compare.append({"Сценарий": r["Сценарий"], "Показатель": "Прибыль", "Значение": r["Прибыль"]})
        df_compare = pd.DataFrame(df_compare)

        st.dataframe(df_compare.style.format({"Значение": "{:,.2f}"}))

        st.write("Ниже — сравнительный график (bar-chart) для трёх сценариев:")
        from streamlit_ui import ChartDisplay
        chart_scen = ChartDisplay("Сравнение сценариев (Доход, Расход, Прибыль)")
        chart_scen.display_bar(df_compare, x="Сценарий", y="Значение", color="Показатель")

        st.success(
            "Таким образом, можно наглядно сравнить показатели в Оптимистичном, Реалистичном и Пессимистичном вариантах."
        )