...
# Sourcery советует объединить if query_params и "params" in query_params
...
with st.sidebar:
    ...
    # Ниже — функционал «Сформировать быстрый отчёт» (был ранее)
    if st.sidebar.button("Сформировать быстрый отчёт"):
        # Пример: сводка по единовременным и ежемесячным расходам.
        one_time_sum = (
    ...
    # [NEW ADVANCED REPORT] Добавляем вторую кнопку: «Сформировать углублённый отчёт»
    if st.sidebar.button("Сформировать углублённый отчёт"):
        # К примеру, сделаем дополнительную аналитику, связанную
        # с разницей между единовременными и ежемесячными расходами.
        one_time_sum2 = (
            app_state.get("one_time_setup_cost")
            + app_state.get("one_time_equipment_cost")
            + app_state.get("one_time_other_costs")
