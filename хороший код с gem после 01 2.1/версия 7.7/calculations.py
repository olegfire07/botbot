...
def calculate_irr(cash_flows):
    ...
    try:
        ...
        # Заменили if/else на тернарный оператор (Sourcery)
...
def calculate_areas(params: WarehouseParams):
    ...
    try:
        ...
        if params.mode == "Ручной":
            ...
            # Объединили проверку: if total_manual > usable_area and usable_area > 0 => if total_manual > usable_area > 0
...
def calculate_total_bep(financials: dict, params: WarehouseParams):
    ...
    try:
        ...
        # Вместо if / return => применили тернарный оператор (Sourcery)
...
def calculate_monthly_bep(financials: dict, params: WarehouseParams):
    ...
    try:
        ...
        # Инлайн сразу в return (Sourcery)
        return pd.DataFrame(
            {
                "Месяц": months,
...
def monte_carlo_simulation(
    ...
    try:
        ...
        # Инлайн сразу return DataFrame (Sourcery)
        return pd.DataFrame(
            {
                "Месяц": months,
...
def calculate_multidimensional_sensitivity(params: WarehouseParams, param_keys: list, param_ranges: dict, disable_extended: bool):
    """
    Проводит многомерный анализ чувствительности по заданным параметрам.
    """
    try:
        results = []
        ...
        df["Параметры"] = df["Параметры"].apply(lambda x: ", ".join([f"{key}={val:.2f}" for key, val in x.items()]))
        for param_key in param_keys:
            df[param_key] = df["Параметры"].apply(lambda x: float(x.split(f"{param_key}=")[1].split(",")[0])  if  f"{param_key}=" in x else np.nan)
        return df
    except Exception as e:
...
def calculate_monthly_projection(params: WarehouseParams, base_financials: dict):
    ...
    try:
        ...
        # Инлайним сразу возврат DataFrame (Sourcery)
        return pd.DataFrame(
            {
                "Месяц": months,
