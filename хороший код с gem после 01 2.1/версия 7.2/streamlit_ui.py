...
def display_tab1(tab, base_financials, pm, pr, roi, irr, params, selected_param=None, main_color="#007bff", npv=0.0):
    display_tab1_header(tab, main_color)
    display_tab1_metrics(tab, base_financials, pm, pr, roi, irr, npv)
    display_tab1_bep(tab, params, base_financials)
    display_tab1_chart(tab, base_financials)
    display_tab1_analysis(tab, base_financials, pm, pr, roi, irr)
    with tab:
        st.markdown("""
    ...
    with tab:
        if enable_ml_settings:
            ...
            if ml_model is not None and df_for_ml is not None:
                with st.spinner("Выполняется прогноз..."):  # Индикация загрузки
                    
                    from ml_models import prepare_ml_data
                    df_prepared = prepare_ml_data(df_for_ml, target_column="Доходы") # Prepare the data again
                    
                    future_months = list(range(1, params.time_horizon + 1))
                    predictions, intervals = predict_with_model(ml_model, df_prepared, future_months, features=params.features, auto_feature_selection=params.auto_feature_selection) #  Получаем предсказания и интервалы
                    df_ml = pd.DataFrame({"Месяц": future_months, "Прогноз Доходы": predictions})
                   
                    if intervals is not None: # Выводим, если есть интервалы
                         ...
                         ChartDisplay("Прогноз доходов (ML)", y_title="Рубли").display_line(
                                df_ml, "Месяц", ["Прогноз Доходы", "Нижняя граница", "Верхняя граница"], color = "Прогноз Доходы", fill_between=("Нижняя граница", "Верхняя граница")
                         )
                    else:
                       ChartDisplay("Прогноз доходов (ML)", y_title="Рубли").display_line(
                    ...
                    st.info(
                        "При ML-прогнозе мы учитываем тренды из исторических данных. "
                        "Линейная/полиномиальная регрессия — для поиска зависимости, "
                        "случайный лес — более сложная модель, но может дать точнее результат."
                           "SVR - для поиска зависимости с нелинейной функцией, XGBoost - модель на основе градиентного бустинга."
                    )
                     # Вывод метрик качества (RMSE)
                    try:
                       ...
                       if params.auto_feature_selection and params.features is not None and len(params.features) > 1:
                           from ml_models import SelectKBest, f_regression
                           ...
                           y_pred = ml_model.predict(df_prepared[selected_features].values) # Используем отфильтрованные признаки
                       else:
                           X = df_prepared[params.features].values # Берем все признаки.
                           y_pred = ml_model.predict(X)

                       y_true = df_prepared["Доходы"].values # Берем все значения y

                       rmse, r2, mae = calculate_metrics(y_true, y_pred) # используем y_pred
                       st.write(f"**RMSE:** {rmse:.2f}, **R2:** {r2:.2f}, **MAE:** {mae:.2f}")

                    except Exception as e:
                         st.warning(f"Ошибка при расчете RMSE:{e}")
                
                # Кнопка для сохранения ML-модели
                if st.button("💾 Сохранить ML-модель", help="Сохранить обученную ML-модель"):
                     from ml_models import save_ml_model
                     model_filename = f"ml_model_{params.forecast_method}_{params.poly_degree}_{params.n_estimators}_{'_'.join(params.features or [])}_{params.param_search_method}_{params.auto_feature_selection}.pkl"
                     save_ml_model(ml_model, model_filename)
                     st.success("ML-модель сохранена.")
            else:
...
def display_tab2_monte_carlo(tab, base_financials, params):
    with tab:
        ...
        num_simulations = min(params.monte_carlo_simulations, 100) # Ограничение для скорости

        if params.monte_carlo_distribution == "Равномерное":
            inc_factors = np.random.uniform(
                1 - params.monte_carlo_deviation, 1 + params.monte_carlo_deviation, (num_simulations, params.time_horizon)
            )
            exp_factors = np.random.uniform(
                1 - params.monte_carlo_deviation, 1 + params.monte_carlo_deviation, (num_simulations, params.time_horizon)
            )
        elif params.monte_carlo_distribution == "Нормальное":
              inc_factors = np.random.normal(params.monte_carlo_normal_mean, params.monte_carlo_normal_std, (num_simulations, params.time_horizon))
              exp_factors = np.random.normal(params.monte_carlo_normal_mean, params.monte_carlo_normal_std, (num_simulations, params.time_horizon))
        elif params.monte_carlo_distribution == "Треугольное":
              inc_factors = np.random.triangular(params.monte_carlo_triang_left, params.monte_carlo_triang_mode, params.monte_carlo_triang_right,(num_simulations, params.time_horizon))
              exp_factors = np.random.triangular(params.monte_carlo_triang_left, params.monte_carlo_triang_mode, params.monte_carlo_triang_right, (num_simulations, params.time_horizon))
        ...
        for i in range(num_simulations):
            df_plot_income[f"Симуляция {i+1}"] = simulated_incomes[i]
        df_plot_income["Месяц"] = months
        df_plot_income["Среднее"] = incomes.flatten()

        df_plot_expenses = pd.DataFrame()
        for i in range(num_simulations):
            df_plot_expenses[f"Симуляция {i+1}"] = simulated_expenses[i]
        df_plot_expenses["Месяц"] = months
        df_plot_expenses["Среднее"] = expenses.flatten()
        
        df_plot_profit = pd.DataFrame()
        for i in range(num_simulations):
            df_plot_profit[f"Симуляция {i+1}"] = simulated_profits[i]
        df_plot_profit["Месяц"] = months
        df_plot_profit["Среднее"] = profits.flatten()
...
def display_tab3_monthly_bep(tab, base_financials, params):
    with tab:
        ...
        st.info("По графику видно, как растет необходимый доход, это нужно учитывать при планировании.")


def display_tab3_sensitivity(tab, params, disable_extended):
    with tab:
        st.write("---")
        ...
        def build_bep_df(p, param_key, base_val, min_val, max_val):
            ...
            return pd.DataFrame({param_key: vals, "Прибыль": profits, "BEP": beps})

        # ПЕРЕВОДИМ ЗДЕСЬ
        # КЛЮЧ ИМЕННО these keys, so user sees them
        analysis_params = {
            # Вопрос, может ли st.multiselect отобразить русские названия?
            # Решение: мы создадим словарь param_display_map (ключ -> русское название),
            # а в st.multiselect() передадим именно русские названия как options,
            # и сделаем reverse map, чтобы понять, какой ключ выбрать.
        }

        # Новый словарь отображения (параметр -> RU)
        param_display_map = {
        ...
        # Обратный словарь (чтобы сопоставлять выбранные русские названия с ключами)
        reverse_map = {v: k for k, v in param_display_map.items()}

        # Список опций на русском
        ru_options = list(param_display_map.values())

        # Выбираем среди русских названий
        ...
        for key in selected_keys:
            ...
            with col1:
                min_val = st.number_input(f"Мин. значение для {param_display_map[key]}", 
                                            value = base_val * 0.5 if base_val > 0 else 0.0,
                                            help="Мин. значение для варьирования параметра",
                                            format = "%.2f", key = f"min_{key}")
            with col2:
                max_val = st.number_input(f"Макс. значение для {param_display_map[key]}", 
                                        value = base_val * 1.5 if base_val > 0 else 1.0,
                                        help="Макс. значение для варьирования параметра",
                                        format = "%.2f", key = f"max_{key}")
            df_sens = build_bep_df(params, key, base_val, min_val, max_val)
            # Выводим RU-название вместо key
            ru_label = param_display_map[key]
            ChartDisplay(f"{ru_label}: чувствительность", x_title=ru_label).display_interactive_line(
                df_sens, key, ["Прибыль", "BEP"], markers = True
            )
            st.info(
                f"На графике видно, как изменения параметра '{ru_label}' "
                f"влияют на прибыль и точку безубыточности. "
                "Если BEP растёт сильнее, чем прибыль, значит рост этого параметра "
                "может ухудшить экономику склада."
            )
        ...
        if len(selected_keys_multidim) > 0:
            param_ranges = {}
            for key in selected_keys_multidim:
               base_val = getattr(params, key)
               col1, col2 = st.columns(2)
               with col1:
                  min_val = st.number_input(f"Мин. для {param_display_map[key]}", 
                                            value = base_val * 0.8 if base_val > 0 else 0.0,
                                            help="Мин. значение",
                                            format = "%.2f", key=f"multidim_min_{key}")
               with col2:
                  max_val = st.number_input(f"Макс. для {param_display_map[key]}", 
                                         value = base_val * 1.2 if base_val > 0 else 1.0,
                                         help="Макс. значение",
                                         format = "%.2f", key=f"multidim_max_{key}")
               param_ranges[key] = np.linspace(min_val, max_val, 3) # Делаем 3 значения (для примера)
            
            scenario_options = ["Базовый", "Оптимистичный", "Пессимистичный"]
            selected_scenario = st.selectbox("Выберите сценарий", scenario_options, help="Выберите предустановленный сценарий", key="scenario_select")
            if st.button("Выполнить многомерный анализ", key="multidim_analyze_btn"):
                ...
                if selected_scenario == "Базовый":
                    df_multi = calculate_multidimensional_sensitivity(params, selected_keys_multidim, param_ranges, disable_extended)
                else: # scenario chosen
                    temp_params = params
                    for param_key in selected_keys_multidim:
                       base_val = getattr(temp_params, param_key)
                       if selected_scenario == "Оптимистичный":
                            setattr(temp_params, param_key, base_val * 1.2 if base_val > 0 else 1.0)
                       elif selected_scenario == "Пессимистичный":
                            setattr(temp_params, param_key, base_val * 0.8 if base_val > 0 else 0.0)
                    df_multi = calculate_multidimensional_sensitivity(temp_params, selected_keys_multidim, param_ranges, disable_extended)

                # Заменяем технические названия на русские (для отображения в таблице)
                for param_key in selected_keys_multidim:
                    df_multi[param_key] = df_multi["Параметры"].apply(lambda x: float(x.split(f"{param_key}=")[1].split(",")[0])  if  f"{param_key}=" in x else np.nan) # Значения float
                
                # Здесь правим названия для таблицы
                df_multi["Параметры"] = df_multi["Параметры"].apply(lambda x: ", ".join([f"{param_display_map.get(k, k)}={v:.2f}" for k, v in zip(x.split(", "),[float(val.split("=")[1]) for val in x.split(", ")])]))
                
                TableDisplay().display(df_multi.copy()) # Выводим таблицу с результатами
                # Визуализация хитмапом, если параметров == 2
                if len(selected_keys_multidim) == 2:
                    key1 = selected_keys_multidim[0]
                    key2 = selected_keys_multidim[1]
                    df_multi_pivot = df_multi.pivot_table(
                        index=key1, columns=key2, values="Прибыль (руб.)"
                    )
                    
                    if not df_multi_pivot.empty:
                        # Создаем copy pivot table для отображения в графике
                        df_multi_pivot_copy = df_multi_pivot.copy()
                        # Заменяем технические названия на русские в индексах и колонках
                        df_multi_pivot_copy.index = [param_display_map.get(k,k) + f" = {str(val)}" for val, k in zip(df_multi_pivot.index, [key1]*len(df_multi_pivot.index))]
                        df_multi_pivot_copy.columns = [param_display_map.get(k,k) + f" = {str(val)}"  for val, k in zip(df_multi_pivot.columns, [key2]*len(df_multi_pivot.columns))]
                        
                        ChartDisplay(f"Прибыль: {param_display_map[key1]} vs {param_display_map[key2]}", x_title=param_display_map[key2], y_title=param_display_map[key1]).display_heatmap(df_multi_pivot_copy)
                    else:
                        st.warning("Нет данных для отображения графика. Попробуйте изменить диапазон параметров.")
                else:
                    ChartDisplay("Прибыль (многомерный анализ)").display_bar(df_multi, x="Параметры", y="Прибыль (руб.)")
            st.info(
...
def display_tab4_results(tab, base_financials, params, irr_val):
    ...
    with tab:
        ...
        st.markdown(generate_download_link(df_results, filename="results.csv"))
        st.markdown(generate_excel_download(df_results, filename="results.xlsx"))
        st.info(
    ...
    with tab:
        ...
        for attr in attributes_to_compare:
            ...
            if current_value != saved_value:
                ru_label = param_display_map.get(attr, attr)  # Use original attr if not found
