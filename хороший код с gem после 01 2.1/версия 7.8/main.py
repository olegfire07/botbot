...
with st.sidebar:
    ...
    with st.sidebar.expander("### Расширенные параметры и прогнозирование", expanded=False):
        ...
        # Убрали XGBoost можно (или оставить?), но здесь оставим чтобы не вырезать
        fm_options = [
            "Базовый",
...
if not is_valid:
    ...
    if chosen == "Вкладка 1":
        ...
        if params.forecast_method == "Базовый":
            ...
            if st.button("Обучить модель"):
                if df_for_ml is not None:
                    ...
                    st.warning("Нет данных для обучения (CSV/Excel не загружен).")

            # Далее стандартный вывод (прогноз и т.д.)
            display_tab2_ml_forecast(
                tab_container,
                params.enable_ml_settings,
                selected_forecast_method,
                st.session_state.get("ml_model"),  # берем актуальную из session_state
                df_for_ml,
                params,
                base_financials
            )
        ...
        # -- ВАЖНО: Графики чувствительности и многомерный анализ --
    ...
    else:  # "Вкладка 5" — Сценарный анализ (пример)
        st.header("Сценарный анализ (Вкладка 5)")
        st.write("Оптимистичный, Реалистичный и Пессимистичный сценарии для ключевых параметров хранения и займов.")
        ...
        scenario_irr = calculate_irr(
            [-temp_params.one_time_setup_cost - temp_params.one_time_equipment_cost
             - temp_params.one_time_other_costs - temp_params.one_time_legal_cost
        ...
        st.write("**Пример**: в оптимистичном сценарии \"storage_fee\" увеличена на 20%, \"loan_interest_rate\" уменьшена на 20%. "
                 "В пессимистичном наоборот. Реалистичный — без изменений.")

        from streamlit_ui import ChartDisplay
        scenario_data = pd.DataFrame({
            "Категория": ["Доход", "Расход"],
            "Значение": [scenario_fin["total_income"], scenario_fin["total_expenses"]]
        })
        chart_display = ChartDisplay(
            f"Доход и Расход ({selected_scenario})",
            color_map={"Доход": "green", "Расход": "red"}
        )
        chart_display.display_bar(scenario_data, "Категория", "Значение")