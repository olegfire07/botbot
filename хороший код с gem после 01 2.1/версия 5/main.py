...
selected_forecast_method = app_state.get("forecast_method", "Базовый")
# poly_degree также, если None, то = 2
poly_degree = app_state.get("poly_degree", 2)
n_estimators = app_state.get("n_estimators", 100)

df_for_ml = app_state.get("df_for_ml")
ml_model = app_state.get("ml_model")
features = app_state.get("features", ["Месяц", "Lag_1", "Lag_2", "Rolling_Mean_3", "Rolling_Mean_5"])
...
def display_parameter_section(title, content_func, expanded=False):
    """Отображает секцию параметров с возможностью сворачивания."""
    with st.sidebar.expander(f"### {title}", expanded=expanded):
        content_func()

def create_total_area_input():
    total_area = st.number_input(
        "📏 Общая площадь (м²)",
        value=app_state.get("total_area", 250),
        step=10,
        min_value=1,
        format="%i",
...
def create_rental_cost_input():
     rental_cost_per_m2 = st.number_input(
            "💰 Стоимость аренды (руб./м²/мес.)",
            value=app_state.get("rental_cost_per_m2", 1000),
            step=50,
            min_value=1,
            format="%i",
...
def create_useful_area_ratio_slider():
        useful_area_ratio_slider = st.slider(
            "📐 Доля полезной площади (%)",
            40,
            80,
            int(app_state.get("useful_area_ratio", 0.7) * 100),
            5,
...
def create_theme_settings():
    ...
    current_theme = app_state.get("selected_theme", "Стандартная")
    ...
    main_color = st.color_picker('Выберите основной цвет', value=app_state.get("main_color", "#007bff"), help="Выберите цвет, который будет использоваться в качестве основного.")
...
def create_manual_area_inputs():
    if app_state.get("mode") == "Ручной":
        st.markdown("#### Ручной ввод площадей (м²)")
        temp_usable = app_state.get("total_area") * app_state.get("useful_area_ratio")

        col1, col2 = st.columns(2)
        storage_area_manual = col1.number_input(
            "Простое",
            value=app_state.get("storage_area_manual", 30.0),
            step=10.0,
            min_value=0.0,
            format="%.2f",
        ...
        loan_area_manual = col2.number_input(
            "Займы",
            value=app_state.get("loan_area_manual", 30.0),
            step=10.0,
            min_value=0.0,
            format="%.2f",
        ...
        vip_area_manual = col3.number_input(
            "VIP",
            value=app_state.get("vip_area_manual", 10.0),
            step=10.0,
            min_value=0.0,
            format="%.2f",
        ...
        short_term_area_manual = col4.number_input(
            "Краткосрочное",
            value=app_state.get("short_term_area_manual", 10.0),
            step=10.0,
            min_value=0.0,
            format="%.2f",
...
def create_tariffs_and_density_inputs():
    storage_fee = st.number_input(
            "💳 Тариф простого (руб./м²/мес.)",
            value=app_state.get("storage_fee", 1500),
            step=100,
            min_value=0,
            format="%i",
    ...
    shelves_per_m2 = col1.number_input(
        "📚 Полок на 1 м²",
        value=app_state.get("shelves_per_m2", 2),
        step=1,
        min_value=1,
        max_value=100,
        format="%i",
    ...
    short_term_daily_rate = col2.number_input(
        ...
        value=app_state.get("short_term_daily_rate", 60.0),
        step=10.0,
        min_value=0.0,
        format="%.2f",
    ...
    vip_extra_fee = st.number_input(
        ...
        value=app_state.get("vip_extra_fee", 1000.0),
        step=50.0,
        min_value=0.0,
        format="%.2f",
...
def create_evaluation_and_loans_inputs():
        item_evaluation_slider = st.slider(
            "🔍 Оценка вещи (%)",
            0,
            100,
            int(app_state.get("item_evaluation", 0.80) * 100),
            5,
        ...
        item_realization_markup = st.number_input(
            "📈 Наценка реализации (%)",
            value=app_state.get("item_realization_markup", 20.0),
            step=5.0,
            min_value=0.0,
            max_value=100.0,
            format="%.1f",
        ...
        average_item_value = st.number_input(
            ...
            value=app_state.get("average_item_value", 15000),
            step=500,
            min_value=0,
            format="%i",
        ...
        loan_interest_rate = st.number_input(
            "💳 Ставка займов (%/день)",
            value=app_state.get("loan_interest_rate", 0.317),
            step=0.01,
            min_value=0.0,
            format="%.3f",
        ...
        loan_term_days = st.number_input(
            ...
            value=app_state.get("loan_term_days", 30),
            step=1,
            min_value=1,
            format="%i",
...
def create_realization_rates_sliders():
    realization_share_storage_slider = st.slider(
            "Простое",
            0,
            100,
            int(app_state.get("realization_share_storage", 0.10) * 100),
            5,
    ...
    realization_share_loan_slider = st.slider(
            ...
            0,
            100,
            int(app_state.get("realization_share_loan", 0.10) * 100),
            5,
    ...
    realization_share_vip_slider = st.slider(
            ...
            0,
            100,
            int(app_state.get("realization_share_vip", 0.10) * 100),
            5,
    ...
    realization_share_short_term_slider = st.slider(
            ...
            0,
            100,
            int(app_state.get("realization_share_short_term", 0.10) * 100),
            5,
...
def create_fill_rates_sliders():
    storage_fill_rate_slider = st.slider(
            "Простое",
            0,
            100,
            int(app_state.get("storage_fill_rate", 1.0) * 100),
            5,
    ...
    loan_fill_rate_slider = st.slider(
        ...
        0,
        100,
        int(app_state.get("loan_fill_rate", 1.0) * 100),
        5,
    ...
    vip_fill_rate_slider = st.slider(
        ...
        0,
        100,
        int(app_state.get("vip_fill_rate", 1.0) * 100),
        5,
    ...
    short_term_fill_rate_slider = st.slider(
        ...
        0,
        100,
        int(app_state.get("short_term_fill_rate", 1.0) * 100),
        5,
...
def create_item_densities_inputs():
    storage_items_density = st.number_input(
            "Простое",
            value=app_state.get("storage_items_density", 1),
            step=1,
            min_value=1,
            format="%i",
    ...
    loan_items_density = st.number_input(
        "Займы",
        value=app_state.get("loan_items_density", 1),
        step=1,
        min_value=1,
        format="%i",
    ...
    vip_items_density = st.number_input(
        "VIP",
        value=app_state.get("vip_items_density", 1),
        step=1,
        min_value=1,
        format="%i",
    ...
    short_term_items_density = st.number_input(
        "Краткосрочное",
        value=app_state.get("short_term_items_density", 1),
        step=1,
        min_value=1,
        format="%i",
...
def create_churn_inputs():
    storage_monthly_churn_num = st.number_input(
        "Простое (%)",
        value=app_state.get("storage_monthly_churn", 0.01) * 100,
        step=0.1,
        min_value=0.0,
        max_value=100.0,
        format="%.1f",
    ...
    loan_monthly_churn_num = st.number_input(
        "Займы (%)",
        value=app_state.get("loan_monthly_churn", 0.02) * 100,
        step=0.1,
        min_value=0.0,
        max_value=100.0,
        format="%.1f",
    ...
    vip_monthly_churn_num = st.number_input(
        "VIP (%)",
        value=app_state.get("vip_monthly_churn", 0.005) * 100,
        step=0.1,
        min_value=0.0,
        max_value=100.0,
        format="%.1f",
    ...
    short_term_monthly_churn_num = st.number_input(
        "Краткосрочное (%)",
        value=app_state.get("short_term_monthly_churn", 0.03) * 100,
        step=0.1,
        min_value=0.0,
        max_value=100.0,
        format="%.1f",
...
def create_monthly_finance_inputs():
    salary_expense = st.number_input(
        "Зарплата (руб./мес.)",
        value=app_state.get("salary_expense", 240000),
        step=10000,
        min_value=0,
        format="%i",
    ...
    miscellaneous_expenses = st.number_input(
        "Прочие (руб./мес.)",
        value=app_state.get("miscellaneous_expenses", 50000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    depreciation_expense = st.number_input(
        "Амортизация (руб./мес.)",
        value=app_state.get("depreciation_expense", 20000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    marketing_expenses = st.number_input(
        "Маркетинг (руб./мес.)",
        value=app_state.get("marketing_expenses", 30000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    insurance_expenses = st.number_input(
        "Страхование (руб./мес.)",
        value=app_state.get("insurance_expenses", 10000),
        step=1000,
        min_value=0,
        format="%i",
    ...
    taxes = st.number_input(
        "Налоги (руб./мес.)",
        value=app_state.get("taxes", 50000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    utilities_expenses = st.number_input(
        "Коммуналка (руб./мес.)",
        value=app_state.get("utilities_expenses", 20000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    maintenance_expenses = st.number_input(
        ...
        value=app_state.get("maintenance_expenses", 15000),
        step=5000,
        min_value=0,
        format="%i",
...
def create_one_time_finance_inputs():
    one_time_setup_cost = st.number_input(
            "Настройка (руб.)",
            value=app_state.get("one_time_setup_cost", 100000),
            step=5000,
            min_value=0,
            format="%i",
    ...
    one_time_equipment_cost = st.number_input(
        "Оборудование (руб.)",
        value=app_state.get("one_time_equipment_cost", 200000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    one_time_other_costs = st.number_input(
        "Другие (руб.)",
        value=app_state.get("one_time_other_costs", 50000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    one_time_legal_cost = st.number_input(
        "Юридические (руб.)",
        value=app_state.get("one_time_legal_cost", 20000),
        step=5000,
        min_value=0,
        format="%i",
    ...
    one_time_logistics_cost = st.number_input(
        "Логистика (руб.)",
        value=app_state.get("one_time_logistics_cost", 30000),
        step=5000,
        min_value=0,
        format="%i",
...
def create_variable_expenses_inputs():
    packaging_cost_per_m2 = st.number_input(
        "Упаковка (руб./м²)",
        value=app_state.get("packaging_cost_per_m2", 10.0),
        step=5.0,
        min_value=0.0,
        format="%.2f",
    ...
    electricity_cost_per_m2 = st.number_input(
        "Электричество (руб./м²)",
        value=app_state.get("electricity_cost_per_m2", 100.0),
        step=10.0,
        min_value=0.0,
        format="%.1f",
...
def create_inflation_and_growth_inputs():
    monthly_inflation_rate_val = st.number_input(
        "Инфляция (%/мес.)",
        value=app_state.get("monthly_inflation_rate", 0.005) * 100,
        step=0.1,
        min_value=0.0,
        format="%.1f",
    ...
    monthly_rent_growth_val = st.number_input(
        "📈 Рост аренды (%/мес.)",
        value=app_state.get("monthly_rent_growth", 0.01) * 100,
        step=0.5,
        min_value=0.0,
        format="%.1f",
    ...
    monthly_salary_growth_val = st.number_input(
        "📈 Рост зарплаты (%/мес.)",
        value=app_state.get("monthly_salary_growth", 0.005) * 100,
        step=0.1,
        min_value=0.0,
        format="%.1f",
...
def create_inflation_and_growth_inputs():
    ...
    monthly_other_expenses_growth_val = st.number_input(
        ...
        value=app_state.get("monthly_other_expenses_growth", 0.005) * 100,
        step=0.1,
        min_value=0.0,
        format="%.1f",
...
def create_extended_settings_inputs():
    disable_extended = st.checkbox(
        "🚫 Отключить расширенные параметры",
        value=app_state.get("disable_extended", False),
    ...
    if not disable_extended:
        time_horizon_val = st.slider(
            ...
            1, 24,
            value=app_state.get("time_horizon", 6),
        ...
        default_probability_val = st.number_input(
            ...
            value=app_state.get("default_probability", 0.05) * 100,
            step=1.0,
            min_value=0.0,
            max_value=100.0,
            format="%.1f",
        ...
        liquidity_factor_val = st.number_input(
            ...
            value=app_state.get("liquidity_factor", 1.0),
            step=0.1,
            min_value=0.1,
            format="%.1f",
        ...
        safety_factor_val = st.number_input(
            "🛡 Коэффициент запаса",
            value=app_state.get("safety_factor", 1.2),
            step=0.1,
            min_value=0.1,
            format="%.1f",
        ...
        loan_grace_period_val = st.number_input(
            "⏳ Льготный период (мес.)",
            value=app_state.get("loan_grace_period", 0),
            step=1,
            min_value=0,
            format="%i",
        ...
        monthly_income_growth_val = st.number_input(
            "📈 Рост доходов (%/мес.)",
            value=app_state.get("monthly_income_growth", 0.0) * 100,
            step=0.5,
            format="%.1f",
        ...
        monthly_expenses_growth_val = st.number_input(
            "📉 Рост расходов (%/мес.)",
            value=app_state.get("monthly_expenses_growth", 0.0) * 100,
            step=0.5,
            format="%.1f",
    ...
    current_fm = app_state.get("forecast_method", "Базовый")
    ...
    if forecast_method_sel == "Симуляция Монте-Карло":
        monte_carlo_simulations_val = st.number_input(
            ...
            value=app_state.get("monte_carlo_simulations", 100),
            step=10,
            min_value=10,
            format="%i",
        ...
        monte_carlo_deviation_val = st.number_input(
            "🔀 Отклонения (0.1 = ±10%)",
            value=app_state.get("monte_carlo_deviation", 0.1),
            step=0.01,
            min_value=0.01,
            format="%.2f",
        ...
        monte_carlo_seed_val = st.number_input(
            "🔑 Seed",
            value=app_state.get("monte_carlo_seed", 42),
            step=1,
            format="%i",
        ...
        # Добавляем выбор типа распределения
        mc_dist_options = ["Равномерное", "Нормальное", "Треугольное"]
        current_dist = app_state.get("monte_carlo_distribution", "Равномерное")
        ...
        # Добавляем поля для параметров распределения
        if monte_carlo_distribution_sel == "Нормальное":
            mc_normal_mean_val = st.number_input(
                "Среднее",
                value=app_state.get("monte_carlo_normal_mean", 0.0),
                step=0.1,
                format="%.1f",
            ...
            mc_normal_std_val = st.number_input(
                "Ст. откл.",
                value=app_state.get("monte_carlo_normal_std", 0.1),
                step=0.01,
                min_value=0.01,
                format="%.2f",
        ...
        if monte_carlo_distribution_sel == "Треугольное":
            mc_triang_left_val = st.number_input(
               "Мин. значение",
               value=app_state.get("monte_carlo_triang_left", 0.0),
               step=0.1,
               format="%.1f",
            ...
            mc_triang_mode_val = st.number_input(
                "Мода",
                value=app_state.get("monte_carlo_triang_mode", 1.0),
                step=0.1,
                format="%.1f",
            ...
            mc_triang_right_val = st.number_input(
                "Макс. значение",
                value=app_state.get("monte_carlo_triang_right", 2.0),
                step=0.1,
                format="%.1f",
    ...
    enable_ml_settings_val = st.checkbox(
        ...
        value=app_state.get("enable_ml_settings", False),
    ...
    if forecast_method_sel == "ML (полиномиальная регрессия)" and enable_ml_settings_val:
        poly_degree_val = st.number_input(
            ...
            min_value=1,
            max_value=5,
            value=app_state.get("poly_degree", 2),
            step=1,
            format="%i",
    ...
    if forecast_method_sel == "ML (случайный лес)" and enable_ml_settings_val:
        n_estimators_val = st.number_input(
            ...
            min_value=10,
            max_value=500,
            value=app_state.get("n_estimators", 100),
            step=10,
            format="%i",
        ...
        selected_features = st.multiselect("Выберите признаки", 
                                            ...
                                            default=app_state.get("features", ["Месяц", "Lag_1", "Lag_2", "Rolling_Mean_3", "Rolling_Mean_5"]),
...
def load_ml_data_from_upload():
    """Загружает данные для ML из загруженного файла."""
    forecast_method = app_state.get("forecast_method", "Базовый")
    enable_ml_settings_val = app_state.get("enable_ml_settings", False)
    if enable_ml_settings_val and forecast_method.startswith("ML"):
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                logging.error(f"Ошибка при загрузке файла ML: {e}")
        else:
            app_state.set("df_for_ml", None)
    else:
        app_state.set("df_for_ml", None)
...
def save_parameters_to_file():
    ...
    if st.sidebar.button("Сохранить в файл"):
        try:
            ...
            logging.error(f"Ошибка при сохранении параметров в файл: {e}")
...
with st.sidebar:
    ...
    # Настройка темы
    display_parameter_section("Настройка темы", create_theme_settings, expanded=False)

    # --- Основные параметры ---
    display_parameter_section("Основные параметры", 
        lambda: (
            create_total_area_input(),
            create_rental_cost_input(),
            create_useful_area_ratio_slider(),
        ), expanded=False)

    # --- Распределение площади ---
    display_parameter_section("Распределение площади", lambda: (create_area_mode_radio(), create_manual_area_inputs()), expanded=False)

    # --- Тарифы и плотности ---
    display_parameter_section("Тарифы и плотности", create_tariffs_and_density_inputs, expanded=False)

    # --- Оценка и займы ---
    display_parameter_section("Оценка и займы", create_evaluation_and_loans_inputs, expanded=False)

    # --- Реализация (%) ---
    display_parameter_section("Реализация (%)", create_realization_rates_sliders, expanded=False)

    # --- Процент заполняемости ---
    display_parameter_section("Процент заполняемости", create_fill_rates_sliders, expanded=False)

    # --- Плотность (вещей/м²) ---
    display_parameter_section("Плотность (вещей/м²)", create_item_densities_inputs, expanded=False)

    # --- Отток клиентов/вещей (%) ---
    display_parameter_section("Отток клиентов/вещей (%)", create_churn_inputs, expanded=False)

   # --- Финансы (ежемесячные) ---
    display_parameter_section("Финансы (ежемесячные)", create_monthly_finance_inputs, expanded=False)

    # --- Финансы (единовременные) ---
    display_parameter_section("Финансы (единовременные)", create_one_time_finance_inputs, expanded=False)

    # --- Переменные расходы (упаковка + электричество) ---
    display_parameter_section("Переменные расходы", create_variable_expenses_inputs, expanded=False)

    # --- Инфляция и рост ---
    display_parameter_section("Инфляция и рост", create_inflation_and_growth_inputs, expanded=False)

    # --- Расширенные параметры ---
    display_parameter_section("Расширенные параметры и прогнозирование", create_extended_settings_inputs, expanded=False)

    # Загрузка данных для ML, если нужно
    load_ml_data_from_upload()
    ...
    if uploaded_file_sess:
        try:
            ...
            logging.error(f"Ошибка при загрузке файла с параметрами: {e}")


    # Выбор сохраненных параметров
    ...
    # Кнопка "Сохранить параметры в файл"
    save_parameters_to_file()

# Теперь формируем WarehouseParams
forecast_method = app_state.get("forecast_method", "Базовый")

# Собираем параметры, которые есть в WarehouseParams
params_keys =  [
    "total_area", "rental_cost_per_m2", "useful_area_ratio", "mode",
    "storage_share", "loan_share", "vip_share", "short_term_share",
    ...
    "short_term_items_density", "storage_fill_rate", "loan_fill_rate",
    "vip_fill_rate", "short_term_fill_rate", "storage_monthly_churn",
    "loan_monthly_churn", "vip_monthly_churn", "short_term_monthly_churn",
    "salary_expense", "miscellaneous_expenses", "depreciation_expense",
    "marketing_expenses", "insurance_expenses", "taxes",
    "utilities_expenses", "maintenance_expenses", "one_time_setup_cost",
    "one_time_equipment_cost", "one_time_other_costs", "one_time_legal_cost",
    "one_time_logistics_cost", "time_horizon", "monthly_rent_growth",
    ...
    "monte_carlo_seed", "enable_ml_settings", "electricity_cost_per_m2",
    "monthly_inflation_rate", "monthly_salary_growth",
    "monthly_other_expenses_growth", "packaging_cost_per_m2",
    "poly_degree", "n_estimators", "features",
...
params_dict = {key: st.session_state[key] for key in params_keys if key in st.session_state}
...
if not is_valid:
    ...
    logging.error(f"Ошибка валидации входных данных: {error_message}")
else:
    # Считаем распределение площадей
    areas = calculate_areas(params)
    params = dataclasses.replace(params, **areas)
    ...
    # IRR
    irr_val = calculate_irr([-params.one_time_expenses] + [base_financials["profit"]]*params.time_horizon)

    # BEP
    bep_val = calculate_total_bep(base_financials, params)
    monthly_bep_values = calculate_monthly_bep(base_financials, params)
    ...
    # Вкладка 1
    # Внесём чуть больше данных, как вы просили, например, добавим info-блок
    st.session_state["extra_info"] = (
        "Здесь вы видите ключевые показатели (ROI, IRR, маржа, рентабельность). "
        "Используйте эти метрики для оценки эффективности склада. Ниже в вкладках — "
        "доп. графики и анализ чувствительности."
    )

    def display_tabs(tab1, tab2_, tab3_, tab4_, base_financials, pm, pr, roi_val, irr_val, params, selected_param, main_color):
       display_tab1(tab1, base_financials, pm, pr, roi_val, irr_val, params, selected_param, main_color)

       # Вкладка 2
       display_tab2_header(tab2_)
       # Проверяем forecast_method
       if params.forecast_method == "Базовый":
           display_tab2_basic_forecast(tab2_, base_financials, params)
       elif params.forecast_method.startswith("ML"):
           # Если есть df_for_ml, подготавливаем и загружаем модель
           if df_for_ml is not None:
               ...
               app_state.set("ml_model", ml_model)
           display_tab2_ml_forecast(
               tab2_,
           ...
           display_tab2_monte_carlo(tab2_, base_financials, params)

       # Вкладка 3
       display_tab3_header(tab3_)
       display_tab3_bep_info(tab3_, base_financials, params)
       display_tab3_monthly_bep(tab3_, base_financials, params)
       display_tab3_sensitivity(tab3_, params, disable_extended=False)

       # Вкладка 4
       display_tab4_header(tab4_)
       display_tab4_area_metrics(tab4_, params)
       display_tab4_storage_table(tab4_, params, base_financials)
       display_tab4_profit_table(tab4_, params, base_financials)
       display_tab4_results(tab4_, base_financials, params, irr_val)

    display_tabs(tab1, tab2_, tab3_, tab4_, base_financials, pm, pr, roi_val, irr_val, params, selected_param, app_state.get("main_color","#007bff"))