...
def display_tab1(tab, base_financials, pm, pr, roi, irr, params, selected_param=None):
    display_tab1_header(tab)
...
def display_tab2_monte_carlo(tab, base_financials, params):
    with tab:
        ...
        df_long_profits = df_mc.melt(
            id_vars="Месяц",
            value_vars=["Средняя Прибыль"],
            var_name="Показатель",
            value_name="Значение",
        )
        ChartDisplay("Прибыль (Монте-Карло)").display_line(df_long_profits, "Месяц", "Значение", color="Показатель")
