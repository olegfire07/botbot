        ...
        if enable_ml_settings:
            ...
            if ml_model is not None and df_for_ml is not None:
                with st.spinner("Выполняется прогноз..."):  # Индикация загрузки
                    ...
                    predictions = predict_with_model(ml_model, df_prepared, future_months, features=params.features, auto_feature_selection=params.auto_feature_selection)
                    df_ml = pd.DataFrame({"Месяц": future_months, "Прогноз Доходы": predictions})
                    st.dataframe(df_ml.style.format({"Прогноз Доходы": "{:,.2f} руб."}))
                    ChartDisplay("Прогноз доходов (ML)").display_line(
                    ...
                    try:
                       ...
                       X = df_prepared[params.features].values # Подготовка X
                       y = df_prepared["Доходы"].values # Подготовка y
                       y_pred = ml_model.predict(X)
                       rmse, r2, mae = calculate_metrics(y, y_pred, features=params.features) # передаем features
