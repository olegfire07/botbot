...
def calculate_multidimensional_sensitivity(params: WarehouseParams, param_keys: list, param_ranges: dict, disable_extended: bool):
    ...
    try:
        ...
        for key, value in original_values.items(): # Возвращаем обратно значения
             setattr(params, key, value)
        
        df = pd.DataFrame(results) # Создаем DataFrame
        df["Параметры"] = df["Параметры"].apply(lambda x: ", ".join([f"{key}={val:.2f}" for key, val in x.items()])) # Строка для показа в таблице и графике
        for param_key in param_keys:
             df[param_key] = df["Параметры"].apply(lambda x: float(x.split(f"{param_key}=")[1].split(",")[0])  if  f"{param_key}=" in x else np.nan) # сохраняем float, а не None
        return df # Возвращаем DataFrame c данными и ключами
    except Exception as e:
