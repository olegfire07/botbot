...
@st.cache_resource
def load_ml_model(df, target_column, model_type="ML (линейная регрессия)", poly_degree=2, n_estimators=100, features=None, param_search_method="Нет", auto_feature_selection=False):
    """
    ...
    try:
        ...
        if df is not None:
            return train_ml_model(df, target_column, model_type, poly_degree, n_estimators, features, param_search_method, auto_feature_selection)
        else:
