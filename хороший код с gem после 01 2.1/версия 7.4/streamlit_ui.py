...
def display_tab1(tab, base_financials, pm, pr, roi_val, irr_val, params, selected_param=None, main_color="#007bff", npv=0.0):
...
def display_tab3_sensitivity(tab, params, disable_extended=False):
    with tab:
        ...
        # Отображение для русских названий параметров
        param_display_map = {
        ...
        # Вспомогательная функция для корректного перевода "param=value"
        def parse_and_translate_params(row_str, pdmap):
            """
            row_str имеет формат "param1=123.45, param2=67.89" (англ. ключи).
            Нужно возвращать строку вида "Тариф хранения=123.45, Наценка VIP=67.89" (русские).
            """
            if not row_str:
                return row_str
            parts = row_str.split(", ")
            ...
            for part in parts:
                if "=" in part:
                    ...
                    v = v.strip()
                    # Переводим k
                    ru_k = pdmap.get(k, k)  # если нет в словаре, оставим как есть
                    # собираем
        ...
        if len(selected_keys_multidim) > 0:
            ...
            selected_scenario = st.selectbox("Выберите сценарий", scenario_options,
            ...
            if st.button("Выполнить многомерный анализ", key="multidim_analyze_btn"):
                ...
                if selected_scenario != "Базовый":
                    for param_key in selected_keys_multidim:
                        ...
                        if selected_scenario == "Оптимистичный":
                            setattr(temp_params, param_key, original_values[param_key] * 1.2 if original_values[param_key] > 0 else 1.0)
                        elif selected_scenario == "Пессимистичный":
                            setattr(temp_params, param_key, original_values[param_key] * 0.8 if original_values[param_key] > 0 else 0.0)

                df_multi = calculate_multidimensional_sensitivity(temp_params, selected_keys_multidim, param_ranges, disable_extended)

                # Восстанавливаем исходные значения после расчётов
                if selected_scenario != "Базовый":
                    for param_key, old_val in original_values.items():
                        setattr(temp_params, param_key, old_val)

                # Переводим технические названия в колонке "Параметры"
                # Т.к. в df_multi["Параметры"] есть строка вида "param1=..., param2=..."
                # Надо подменить ключи "storage_fee" на "Тариф хранения (руб./м²)" и т.д.
                reverse_eng_ru_map = {}
                for eng, ru in param_display_map.items():
                    reverse_eng_ru_map[eng] = ru
                ...
                if len(selected_keys_multidim) == 2:
                    ...
                    if not df_multi_pivot.empty:
                        # Преобразуем индексы и столбцы в русские названия
                        df_multi_pivot_copy = df_multi_pivot.copy()
                        df_multi_pivot_copy.index = [
                            f"{param_display_map[key1]} = {val:.2f}" for val in df_multi_pivot.index
                        ]
                        df_multi_pivot_copy.columns = [
                            f"{param_display_map[key2]} = {val:.2f}" for val in df_multi_pivot.columns
                        ]
                        ChartDisplay(
