...
def display_tab1_metrics(tab, base_financials, profit_margin, profitability, roi, irr):
    with tab:
...
def display_tab1(tab, base_financials, pm, pr, roi, irr, params, selected_param=None, main_color="#007bff"):
    display_tab1_header(tab, main_color)
    display_tab1_metrics(tab, base_financials, pm, pr, roi, irr)
...
def display_tab2_basic_forecast(tab, base_financials, params):
    with tab:
        ...
        df_long = df_projection.melt(
            id_vars="Месяц",
            value_vars=["Доходы", "Расходы", "Прибыль"],
            var_name="Показатель",
            value_name="Значение"
        )
...
def display_tab2_ml_forecast(
    ...
    with tab:
        if enable_ml_settings:
            ...
            if ml_model is not None and df_prepared is not None:
                future_months = list(range(params.time_horizon + 1, params.time_horizon + 7))
                ...
                try:
                    ...
                    X = df_prepared[["Месяц"]].values # Подготовка X
                    y = df_prepared["Доходы"].values # Подготовка y
                    y_pred = ml_model.predict(X)
                    rmse, r2, mae = calculate_metrics(y, y_pred)
...
def display_tab3_header(tab):
    with tab:
        ...
        st.info(
            "Точка безубыточности — момент, когда доходы покрывают все расходы. "
            "Здесь вы узнаете, какой доход нужен, чтобы склад не был убыточен."
        )
...
def display_tab3_monthly_bep(tab, base_financials, params):
    with tab:
        ...
        ChartDisplay("Помесячная BEP").display_line(
            ...
            color="Необходимый доход для BEP"
        )
...
def display_tab3_sensitivity(tab, params, disable_extended):
    with tab:
        ...
        selected_ru_params = st.multiselect(
            ...
            ru_options,
            help="Укажите, какие параметры варьировать, чтобы посмотреть влияние на финрезультат.",
        )
        ...
        for key in selected_keys:
            ...
            ChartDisplay(f"{ru_label}: чувствительность").display_line(
                df_sens, key, ["Прибыль", "BEP"]
            )
            st.info(
...
def display_tab4_results(tab, base_financials, params, irr_val):
    from calculations import calculate_additional_metrics, calculate_roi, calculate_total_bep
    ...
    with tab:
        ...
        st.info(
            ...
            "Здесь вы видите дополнительные метрики: ROI, IRR, "
...
def generate_html_report(base_financials, params, pm, pr, roi, irr, selected_param=None):
    ...
    html_content = f"""
        ...
        <body>
        <h1>Экономическая модель склада</h1>
        <h2>Общие результаты</h2>
        <p><b>Общий доход:</b> {base_financials['total_income']:,.2f} руб.</p>
        <p><b>Общие расходы:</b> {base_financials['total_expenses']:,.2f} руб.</p>
        <p><b>Прибыль:</b> {base_financials['profit']:,.2f} руб.</p>
        <p><b>Маржа прибыли:</b> {pm:.2f}%</p>
        <p><b>Рентабельность:</b> {pr:.2f}%</p>
         <p><b>ROI:</b> {roi:.2f}%</p>
         <p><b>IRR:</b> {irr:.2f}%</p>
    """
    if selected_param:
        ...
        html_content += "<h2>Сравнение параметров</h2>"
        ...
        for attr in attributes_to_compare:
            ...
            if current_value != saved_value:
                ru_label = param_display_map.get(attr, attr)  # Use original attr if not found
                if isinstance(current_value, (int, float)) and isinstance(saved_value, (int, float)):
                     html_content +=  f"<p><b>{ru_label}:</b> Сохранённое: {saved_value:,.2f} | Текущее: {current_value:,.2f}</p>"
                else:
                    html_content +=  f"<p><b>{ru_label}:</b> Сохранённое: {saved_value} | Текущее: {current_value}</p>"
